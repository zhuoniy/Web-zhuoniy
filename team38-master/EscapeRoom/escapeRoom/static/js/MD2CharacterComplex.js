...
THREE.MD2CharacterComplex = function () {
    ...
    function itemMap(newx, newz) {
        ...
        // if (x > 90 && z > -100) //2
        // 	alert("here2");
        if (newx < 61 && newx > 60 && newz < -91) {
            bed();
        } //3 bed
        if (newx > 165.0 && newx < 200 && newz > -160 && newz < -113) {
            door();
        }
        if (newx < -28 && newz < -40) {
            cabinet();
        ...
        if (newx > -25 && newz > 60 || newx < -25 && newz < -9) {
            desk();
        } //5 pc desk

        if (newx > 39 && newx < 79.74 && newz > -20 && newz < 27) {
            closet();
        } //closet
