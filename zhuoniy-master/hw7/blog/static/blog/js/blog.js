...
"<div class='panel panel-default'>" +
    ...
    "<div class='row'>" +
        ...
        "</div>"+
        "<div class='col-md-2'>"+
            "<button class='btn btn-default' onclick='addComment("+id+")'>Add comment</button>"+
        "</div>"+
    "</div>"+
    "</div>"+
    ...
    "</div> "
            ...
            $("#postlist input").blur(function(){
                UpdateSet=window.clearInterval(UpdateSet);
                UpdateSet=window.setInterval(getList, 5000);
            });
...
//1
function addPost() {
    var postElement = $("#text");
    var postValue = postElement.val();

    console.log(postElement);
    console.log("addPost:"+postValue);  
    ...
    $.ajax({
        url: "/blog/add_post",
        type: "POST", 
        data: "text="+postValue+"&csrfmiddlewaretoken="+getCSRFToken(),
        dataType : "json",
        success: function(response) {
...
window.UpdateSet=setInterval(getList, 5000);